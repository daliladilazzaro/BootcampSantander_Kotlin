package com.example.devintegrado.database
...
import com.example.devintegrado.database.NotesDatabaseHelper.Companion.TABLE_NOTES

class NotesProvider : ContentProvider() {

    //responsavel por fazer a validação da URL
    ...
    override fun delete(uri: Uri, selection: String?, selectionArgs: Array<String>?): Int {
        if (mUriMatcher.match(uri) == NOTES_BY_ID){
            ...
            throw UnsupportedSchemeException("Uri inválida para excluir!")
        }
    }
    ...
    override fun insert(uri: Uri, values: ContentValues?): Uri? {
         if (mUriMatcher.match(uri) == NOTES){
             ...
             throw UnsupportedSchemeException("Uri inválida para inserir!")
         }
    }
    ...
    override fun update(uri: Uri, values: ContentValues?, selection: String?,
        ...
        if (mUriMatcher.match(uri) == NOTES_BY_ID){
            val db: SQLiteDatabase = dbHelper.writableDatabase
            val lineAffeted = db.update(TABLE_NOTES, values, "$_ID = ?",
    ...
    companion object{
        const val AUTHORITY= "com.example.devintegrado.provider"
